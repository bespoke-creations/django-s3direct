...
const setFormName = (name) => {
  const name_input = document.querySelector(
    "#videoUploadForm input[name=name]"
  );
  if (name_input !== null) {
...
const setFormKey = (key) => {
  const key_input = document.querySelector("#videoUploadForm input[name=key]");
  if (key_input !== null) {
    key_input.value = key;
  }
};
  ...
  request("POST", destCheckUrl, form, headers, element, (status, response) => {
    ...
    switch (status) {
      case 200:
        ...
        setFormKey(uploadParameters.uuid_key);
